...
namespace eng
{
    ...
    bool Scene::SetParent(GameObject* obj, GameObject* parent)
    {
        ...
        if (parent == nullptr)
        {
            ...
            else //no parent currently.
                //this can happen when object has just been created or is attached to the scene root
            {
                auto it = std::find_if(
                    m_objects.begin(),
                    m_objects.end(),
        ...
        // we are trying to add as a child of another object
        else
        {
            if (currentParent != nullptr)
            {
